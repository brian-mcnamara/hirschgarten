...
import org.jetbrains.bazel.server.sync.languages.java.JavaManifestUtil
      ...
      params.writer().use { os ->
        ...
        while (line != null) {
          ...
          when (line) {
            ...
            "--StatsfileOutput" -> {
              os.writeLn(line)
              os.writeLn(tempDir.resolve("${outputJar.name}.statsfile").pathString)
              ips.readLine()
            }
