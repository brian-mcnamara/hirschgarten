  ...
  override fun run(
    ...
    BazelCoroutineService.getInstance(project).startAsync {
      ...
      moduleBuildTasks.flatMap { it.files.toList() }.forEach { file ->
        ...
        val targetId = targetUtils.getTargetsForFile(file).first()
        val buildCommand = project.connection.runWithServer {
          it.fastBuildFile(FastBuildParams(targetId, file.toNioPath(), tempDir))
        } ?: TODO()
        val tempFastBuildDir = JvmFastBuildClasspathProvider.getInstance().getFastBuildOutputPath(targetId)
